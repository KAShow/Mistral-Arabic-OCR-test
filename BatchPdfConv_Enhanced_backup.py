...
# حدود حجم الملفات (يمكن تعديلها حسب الحاجة)
MAX_FILE_SIZE_MB = 120  # الحد الأقصى لحجم الملف الأصلي
MAX_BASE64_SIZE_MB = 120  # الحد الأقصى لحجم base64
...
class EnhancedPDFProcessor:
    ...
    def __init__(self):
        ...
        print(f"📏 الحد الأقصى لحجم الملف: {MAX_FILE_SIZE_MB} MB")
        print(f"📊 الحد الأقصى لـ base64: {MAX_BASE64_SIZE_MB} MB")
    
    @staticmethod
    def set_size_limits(file_size_mb: int, base64_size_mb: int = None):
        """
        تعديل حدود حجم الملفات
        
        Args:
            file_size_mb: الحد الأقصى لحجم الملف الأصلي بالميجابايت
            base64_size_mb: الحد الأقصى لحجم base64 بالميجابايت (افتراضي نفس حجم الملف)
        """
        global MAX_FILE_SIZE_MB, MAX_BASE64_SIZE_MB
        MAX_FILE_SIZE_MB = file_size_mb
        MAX_BASE64_SIZE_MB = base64_size_mb or file_size_mb
        print(f"✅ تم تحديث الحدود:")
        print(f"   📏 الحد الأقصى لحجم الملف: {MAX_FILE_SIZE_MB} MB")
        print(f"   📊 الحد الأقصى لـ base64: {MAX_BASE64_SIZE_MB} MB")
    ...
    def process_pdf_with_ocr(self, pdf_filename):
        ...
        try:
            ...
            if file_size > MAX_FILE_SIZE_MB * 1024 * 1024:  # استخدام المتغير العام
                raise ValueError(f"الملف كبير جداً: {file_size/1024/1024:.1f} MB (الحد الأقصى: {MAX_FILE_SIZE_MB} MB)")
        ...
        try:
            ...
            max_size_chars = MAX_BASE64_SIZE_MB * 1024 * 1024 * 4 // 3  # تحويل تقريبي لـ base64
...
def main():
    ...
    parser.add_argument('--max-size', type=int, default=120,
                       help='الحد الأقصى لحجم الملف بالميجابايت (افتراضي: 120)')
    parser.add_argument('--max-base64-size', type=int,
                       help='الحد الأقصى لـ base64 بالميجابايت (افتراضي: نفس حجم الملف)')
    ...
    # تعديل الحدود إذا تم تحديدها
    if args.max_size != 120 or args.max_base64_size:
        EnhancedPDFProcessor.set_size_limits(args.max_size, args.max_base64_size)
        print()
